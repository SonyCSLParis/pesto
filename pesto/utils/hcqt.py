...
class CQTv2(nn.Module):
    ...
    center : bool
        Putting the CQT kernel at the center of the time-step or not. If ``False``, the time index is
        the beginning of the CQT kernel, if ``True``, the time index is the center of the CQT kernel.
        Default value if ``True``.
    ...
    def __init__(
        ...
        if self.center:
            if self.pad_mode == "constant":
               self.padding = nn.ConstantPad1d(self.kernel_width // 2, 0)
            elif self.pad_mode == "reflect":
                self.padding = nn.ReflectionPad1d(self.kernel_width // 2)
        else:  # no padding
            self.padding = nn.Identity()

        self.conv_real = cc.Conv1d(1, n_bins, kernel_size=self.kernel_width, stride=self.hop_length, bias=False)
        self.conv_imag = cc.Conv1d(1, n_bins, kernel_size=self.kernel_width, stride=self.hop_length, bias=False)
    ...
    def __init__(
        ...
        self.cqt_kernels = nn.ModuleList([
            CQT(sr=sr, hop_length=hop_length, fmin=h*fmin, fmax=fmax, n_bins=n_bins,
                bins_per_octave=12*bins_per_semitone, gamma=gamma, output_format="Complex")
            for h in harmonics
        ])
